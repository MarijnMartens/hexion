...
class Welcome extends CI_Controller {
    ...
    public function contact($error = NULL) {
        ...
        //Get values where in session for input fiels form
        if ($this->session->userdata('fName')) {
            $nameField = $this->session->userdata('fName') . ' ' . $this->session->userdata('lName');
            $emailField = $this->session->userdata('email');
        } else { //no values but placeholder
            $nameField = null;
            $emailField = null;
        }
        //Prepare form fields
